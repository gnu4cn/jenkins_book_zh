...
[如上]() 所述，`<docker-container-name>` 可以通过 `docker container ls` 命令获得。如果你在上面的 `docker container run ...` 命令中指定了 ``--name jenkins-blueocean` 选项（参见 [访问 Jenkins/Blue Ocean Docker容器]()），咱们可以简单地使用 `docker container exec` 命令：
