...
通常，当咱们在咱们流水线中运行 `stash` 步骤时，无论流水线结果如何，产生的成品暂存（stash）都会在流水线完成时被清空。由于 `stash` 成品在创建他们的流水线运行之外是不可访问的，这对使用并未产生任何限制。但是在声明式阶段重启时，咱们可能希望能够 `unstash` 咱们的重启阶段前运行阶段中的成品。
